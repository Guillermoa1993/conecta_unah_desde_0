...
export function FichaEmpleado() {
  ...
  const handleVerifyCarnet = async () => {
    ...
    try {
      ...
      // Comparación de número de empleado
      const employeeNumClean = formData.numeroEmpleado.trim();
      const employeeNumMatchOk = cleanText.replace(/[\s-]/g, '').includes(employeeNumClean.replace(/[\s-]/g, ''));

      // Comparación de facultad
      const facultyClean = formData.facultad.toLowerCase();
      const facultyWords = facultyClean.split(/\s+/).filter(w => w.length > 4);
      let facultyMatches = 0;
      facultyWords.forEach(word => {
        if (cleanText.includes(word)) facultyMatches++;
      });
      const facultyMatchOk = facultyWords.length > 0 ? (facultyMatches / facultyWords.length) >= 0.5 : true;

      let dataMatchCount = 0;
      if (nameMatchOk) dataMatchCount++;
      if (employeeNumMatchOk) dataMatchCount++;
      if (facultyMatchOk) dataMatchCount++;

      const dataMatchScore = Math.round((dataMatchCount / 3) * 100);

      if (!employeeNumMatchOk) {
        detectedErrors.push(
          `Número de Empleado incorrecto: el número ingresado (${formData.numeroEmpleado}) no coincide con el detectado en el carnet.`
        );
      }
      if (!nameMatchOk) {
        detectedErrors.push(
          `Nombre de Empleado incorrecto: el nombre registrado (${formData.nombre}) no coincide con el leído en el carnet (se requiere al menos un nombre y un apellido correctos).`
        );
      }
      if (!facultyMatchOk) {
        detectedErrors.push(
          `Facultad/Dependencia incorrecta: la facultad seleccionada (${formData.facultad}) no coincide con la indicada en el carnet.`
        );
      }
      ...
      // El carnet se aprueba si coincide el número de empleado, el nombre (mínimo un nombre y un apellido) y la imagen facial
      const shouldApprove = employeeNumMatchOk && nameMatchOk && faceMatchOk;
              ...
              <CardContent className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <option value="">Selecciona tu facultad / dependencia...</option>
                      {UNAH_FACULTADES.map((c) => (
                  ...
                  <div className="lg:col-span-7 space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        {forma003 ? 'Cambiar imagen' : 'Seleccionar Carnet de Empleado'}
                      </label>
                      {forma003 && (
                        <button
                          type="button"
