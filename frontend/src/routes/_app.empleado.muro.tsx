...
export const Route = createFileRoute("/_app/empleado/muro")({
  component: MuroPage,
});
...
function MuroPage() {
  ...
  const [stories, setStories] = useState<typeof pumitasStories>(() => {
    const saved = typeof window !== "undefined" ? localStorage.getItem("unah_empleado_stories") : null;
    if (saved) { try { return JSON.parse(saved); } catch {} }
    return [...pumitasStories];
  });
  const [showCreateStory, setShowCreateStory] = useState(false);
  useEffect(() => { localStorage.setItem("unah_empleado_stories", JSON.stringify(stories)); }, [stories]);
  ...
  return (
    <div className="muro-page">
      <style>{`
        ...
        @media (max-width:768px){.muro-page .btn-primary{font-size:11px;padding:6px 10px;}}
      ...
      <div className="feed-wrapper">
        <div className="feed-topbar">
          ...
          <div className="avatar-btn" onClick={()=>showToast("Configuración de Perfil")}>YO</div>
        </div>
        <StoriesBar onCreateStory={() => setShowCreateStory(true)} />
        <div className="feed-content">
            <div className="controls-row" style={{flexWrap:"wrap"}}>
              <div className="filter-tabs">{["Todas","Evento","Publicacion"].map(f=>(<button key={f} className={`filter-btn${activeFilter===f?" active":""}`} onClick={()=>setActiveFilter(f)}>{f==="Publicacion"?"Publicaciones":f}</button>))}</div>
              <select className="sort-select" value={sortValue} onChange={e=>setSortValue(e.target.value)}><option value="reciente">Más recientes</option><option value="popular">Más populares</option><option value="comentado">Más comentados</option></select>
            </div>
          {filtered.length === 0 ? (
