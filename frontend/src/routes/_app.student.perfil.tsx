...
export const Route = createFileRoute("/_app/student/perfil")({
...
const perfilInicial: PerfilData = {
  ...
  biografia: "Estudiante de la Licenciatura en Administración. Me interesa el análisis de datos, la gestión empresarial y la innovación.",
  horasAcumuladas: 42,
  dobleFactor: true,
  forma003: "",
};
...
const notificacionesRecientes = [
  { icon: "fa-calendar-check", texto: "Tutoría de Finanzas confirmada", tiempo: "Hace 10 min" },
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 text-center shadow-sm">
                <div className="relative w-32 h-32 mx-auto mb-4">
                  <div className="w-full h-full rounded-full border-4 border-[#FFD100] bg-[#F4F6F8] flex items-center justify-center overflow-hidden">
                    {fotoPerfil ? <img src={fotoPerfil} alt="Perfil" className="w-full h-full object-cover" /> : <span className="text-[#004B87] text-4xl font-bold">VE</span>}
                  </div>
    ...
    <div className="space-y-6 bg-[#F4F6F8] min-h-screen">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-xl border border-gray-200 shadow-2xl p-5">
            ...
            <div className="space-y-4">
              ...
              <label className="block"><span className="block text-xs text-[#717182] font-bold uppercase tracking-wide mb-2">Nueva carrera</span><input value={nuevaCarrera} onChange={(e) => setNuevaCarrera(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-3 text-sm text-[#003366] outline-none focus:border-[#004B87]" placeholder="Ej. Ingeniería Industrial" /></label>
              <label className="block"><span className="block text-xs text-[#717182] font-bold uppercase tracking-wide mb-2">Centro regional</span><input value={centroRegionalCambio} onChange={(e) => setCentroRegionalCambio(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-3 text-sm text-[#003366] outline-none focus:border-[#004B87]" placeholder="Ej. UNAH-CU" /></label>
              <label className="block"><span className="block text-xs text-[#717182] font-bold uppercase tracking-wide mb-2">Motivo del cambio</span><textarea value={motivoCambioCarrera} onChange={(e) => setMotivoCambioCarrera(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-3 text-sm text-[#003366] outline-none focus:border-[#004B87] min-h-28 resize-none" placeholder="Explica brevemente el motivo..." /></label>
            </div>
            {errorCambioCarrera && <p className="mt-3 rounded-lg border border-[#D4183D]/20 bg-[#D4183D]/10 px-3 py-2 text-xs font-semibold text-[#D4183D]">{errorCambioCarrera}</p>}
            {mensajeCambioCarrera && <p className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700">{mensajeCambioCarrera}</p>}
            <button type="button" onClick={enviarCambioCarrera} className="mt-5 w-full bg-[#FFD100] text-[#003366] font-bold py-3 rounded-lg hover:bg-[#FFE766] transition-colors">Enviar solicitud</button>
          </div>
        </div>
      )}
      ...
      {mostrarRegistroAcademico && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-xl border border-gray-200 shadow-2xl p-5">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div><h3 className="text-xl font-bold text-[#003366]">Agregar registro académico</h3><p className="text-sm text-[#717182]">Carga Carnet y Forma 003.</p></div>
              <button type="button" onClick={() => setMostrarRegistroAcademico(false)} className="w-9 h-9 rounded-lg bg-[#F4F6F8] border border-gray-200 text-[#003366] hover:bg-[#FFD100] transition-colors"><i className="fa-solid fa-xmark" /></button>
            </div>
            <div className="space-y-4">
              <label className="block"><span className="block text-xs text-[#717182] font-bold uppercase tracking-wide mb-2">Período académico</span><input value={periodoRegistro} onChange={(e) => setPeriodoRegistro(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-3 text-sm text-[#003366] outline-none focus:border-[#004B87]" placeholder="Ej. III PAC 2026" /></label>
              <label className="block cursor-pointer rounded-lg border border-gray-200 bg-[#F4F6F8] p-4 hover:border-[#FFD100] transition-colors"><span className="block text-xs text-[#717182] font-bold uppercase tracking-wide mb-1">Cargar Carnet</span><span className="text-sm font-semibold text-[#003366]">{carnetRegistro || "PDF, JPG o PNG máximo 10 MB"}</span><input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={(e) => manejarSeleccionRegistro("Carnet", e)} /></label>
              <label className="block cursor-pointer rounded-lg border border-gray-200 bg-[#F4F6F8] p-4 hover:border-[#FFD100] transition-colors"><span className="block text-xs text-[#717182] font-bold uppercase tracking-wide mb-1">Cargar Forma 003</span><span className="text-sm font-semibold text-[#003366]">{forma003Registro || "PDF, JPG o PNG máximo 10 MB"}</span><input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={(e) => manejarSeleccionRegistro("Forma 003", e)} /></label>
            </div>
            {mensajeDocumento && <p className="mt-3 rounded-lg border border-[#FFD100]/40 bg-[#FFD100]/10 px-3 py-2 text-xs font-semibold text-[#003366]">{mensajeDocumento}</p>}
            <button type="button" onClick={agregarRegistroAcademico} className="mt-5 w-full bg-[#FFD100] text-[#003366] font-bold py-3 rounded-lg hover:bg-[#FFE766] transition-colors">Validar registro</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-xl border border-gray-200 shadow-2xl">
            <div className="flex items-start justify-between gap-4 p-5 border-b border-gray-100">
              <div><h3 className="text-xl font-bold text-[#003366]">Agregar Pumita a mi red</h3><p className="text-sm text-[#717182]">Sugerencias para conectar.</p></div>
              <button type="button" onClick={() => setMostrarAgregarPumita(false)} className="w-9 h-9 rounded-lg bg-[#F4F6F8] border border-gray-200 text-[#003366] hover:bg-[#FFD100] transition-colors"><i className="fa-solid fa-xmark" /></button>
            </div>
            <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
              {pumitas.filter((p) => p.estado !== "Conectado").map((pumita) => {
                const solicitudLocal = solicitudesEnviadas.includes(pumita.nombre);
                const solicitudEnviada = solicitudLocal || pumita.estado === "Pendiente";
                return (
                  <article key={`agregar-${pumita.nombre}`} className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
                    <img src={pumita.avatar} alt={pumita.nombre} className="w-12 h-12 rounded-full object-cover border border-gray-200" />
                    <div className="min-w-0 flex-1"><h4 className="font-bold text-[#003366] truncate">{pumita.nombre}</h4><p className="text-xs text-[#717182] truncate">{pumita.carrera}</p>{solicitudEnviada && <span className="inline-flex mt-2 px-2 py-0.5 rounded-full border border-[#FFD100]/40 bg-[#FFD100]/20 text-[11px] font-bold text-[#003366]">Pendiente</span>}</div>
                    <div className="flex flex-col gap-2 shrink-0">
                      <button type="button" onClick={() => enviarSolicitudPumita(pumita.nombre)} disabled={solicitudEnviada} className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors ${solicitudEnviada ? "bg-[#F4F6F8] text-[#717182] border border-gray-200 cursor-not-allowed" : "bg-[#FFD100] text-[#003366] hover:bg-[#FFE766]"}`}>{solicitudEnviada ? "Solicitud enviada" : "Agregar"}</button>
                      {solicitudLocal && <button type="button" onClick={() => cancelarSolicitudPumita(pumita.nombre)} className="px-3 py-2 rounded-lg border border-[#D4183D]/20 text-xs font-bold text-[#D4183D] hover:bg-[#D4183D]/10 transition-colors">Cancelar solicitud</button>}
                    </div>
                  </article>
                );
              })}
